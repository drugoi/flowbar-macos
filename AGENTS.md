...
- `Tests/` contains UI tests (`Tests/LongPlayUITests`).
- `Resources/` stores build-fetched binaries (see Build section).

## Build, Test, and Development Commands
- Generate the Xcode project: `xcodegen generate`
- Build: `xcodebuild -scheme LongPlay -configuration Debug -derivedDataPath build`
- Test: `xcodebuild -scheme LongPlay -destination 'platform=macOS' test`
...
## Testing Guidelines
- UI tests live in `Tests/LongPlayUITests` (XCTest).
